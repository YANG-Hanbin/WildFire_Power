...
for p in pList
  for i in 1:1
      ...
      prob[41] = (1 - p) + prob[length(keys(Ω_rv))] #empty scenario
  ...
  save("src/NumericalResults/ProbTest/gurobiResultList.jld2", "gurobiResultList", gurobiResultList)
end
...
for alpha in  pList 
    ...
    state_variable = gurobiResultList[(alpha, k)].first_state_variable;
    ...
    # sum(costDistribution[alpha, ω].damageCost for ω in indexSets.Ω)/5000
    # sum(costDistribution[alpha, ω].loadShedding for ω in indexSets.Ω)/5000


    u = state_value + c;
    totalCost[alpha] = u;
    save("src/NumericalResults/ProbTest/totalCost.jld2", "totalCost", totalCost)
...
save("src/NumericalResults/ProbTest/costDistribution.jld2", "costDistribution", costDistribution)
costDistribution = load("src/NumericalResults/ProbTest/costDistribution.jld2")["costDistribution"]


for alpha in pList
    @info "$alpha, $(costDistribution[alpha, 1090])"
end
