function RobustOptimizationTest!(indexSets::IndexSets, 
                            paramDemand::ParamDemand, 
                            paramOPF::ParamOPF, 
                            Ω_rv::Dict{Int64, RandomVariables}; 
                            timelimit::Int64 = 6000, 
                            mipGap::Float64 = 5e-3)  
    ...
    @constraint(model, [ω in Ω], η ≥ ( sum( sum(paramDemand.w[d] * (1 - x[d, t]) for d in D) for t in 1:Ω_rv[ω].τ - 1 ) + ## 3a
