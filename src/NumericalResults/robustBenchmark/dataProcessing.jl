## This test is design to compare with scenario-based robust optimization model
...
for ω in indexSets.Ω 
    prob[ω] = 1/length(keys(Ω_rv));
end


gurobiResult = RobustOptimizationTest!(indexSets, 
                                    paramDemand, 
                                    paramOPF, 
                                    Ω_rv; 
                                    mipGap = 1e-2, timelimit = 2e4); 


save("src/NumericalResults/robustBenchmark/gurobiResult.jld2", "gurobiResult", gurobiResult)
...
costDistribution = Dict{Int64, NamedTuple{(:damageCost, :loadShedding), Tuple{Float64, Float64}}}()
gurobiResult = load("src/NumericalResults/robustBenchmark/gurobiResult.jld2")["gurobiResult"]
indexSets = load("data/testData_RTS/indexSets.jld2")["indexSets"]
paramOPF = load("data/testData_RTS/paramOPF.jld2")["paramOPF"]
paramDemand = load("data/testData_RTS/paramDemand.jld2")["paramDemand"]
...
state_variable = gurobiResult.first_state_variable;
...
for ω in indexSets.Ω
    ...
    costDistribution[ω] = (damageCost = damageCost, loadShedding = loadShedding)
end
u = state_value + c;
totalCost[1] = u;
save("src/NumericalResults/robustBenchmark/totalCost.jld2", "totalCost", totalCost)
save("src/NumericalResults/robustBenchmark/costDistribution.jld2", "costDistribution", costDistribution)

costDistribution = load("src/NumericalResults/robustBenchmark/costDistribution.jld2")["costDistribution"]
totalCost = load("src/NumericalResults/robustBenchmark/totalCost.jld2")["totalCost"]


@info "$(costDistribution[1090])"
round((sum(costDistribution[ω].damageCost for ω in 1:5000) - costDistribution[1090].damageCost * 103)/(5000 - 103), digits = 1)
round((sum(costDistribution[ω].loadShedding for ω in 1:5000) - costDistribution[1090].loadShedding * 103)/(5000 - 103), digits = 1)
